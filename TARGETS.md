...
| Incremental planar collinearity scores | The 16 no-three-in-line extension and repair programs in the 2026-08-23 scan that independently rescore small point-set edits | Exact collinear-triple totals, per-point conflict degrees, and ragged add/delete/swap scores for unique int32 point sets through 512 base points, using widened exact determinants and deterministic static edit partitioning. A positive cutoff returns flagged capped scores for early-exit searches; otherwise native and executable reference outputs agree completely. On the retained 122-point repair shape with 2,000 two-point swaps, full score arrays fell from 0.994786 s in the scratch program's complete triple audit to 0.0256644 s native (38.8x, 32 workers; 6.18x on one worker), with checksum 133,650,047 unchanged. |
| Packed cyclic correlation profiles | The 19 qLP64 programs in the 2026-08-23 scan that independently rotate masks and rebuild periodic profiles | Batched exact `popcount(mask & rotate_left(mask, lag))` and derived `{+1,-1}` autocorrelations for uint64 cycles of length 1 through 64, with full mask-major lag rows, exact opposite-lag symmetry, out-of-width rejection, and deterministic static row partitioning. The executable reference evaluates every lag directly; native computes each opposite-lag pair once. On gmktec, complete profiles for 500,000 width-32 weight-16 masks fell from 0.00884533 s in the retained packed-key qLP64 C++ loop to 0.000966578 s native (9.15x, 32 workers; 1.18x on one worker), with all 16,000,000 entries in each output equal. |
